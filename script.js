...
function highlightTab(tabName) {
  ...
  function createTable(data) {
    ...
    // Создаем строки данных таблицы
    data.forEach((row, rowIndex) => {
        const tr = document.createElement('tr');
        row.forEach((cell, colIndex) => {
            const td = document.createElement('td');
            td.textContent = (cell !== null && cell !== undefined) ? cell.toString().trim() : '';
            tr.appendChild(td);
        });
    ...
    // Добавляем собранные элементы в таблицу
    ...
    // Устанавливаем обработчики кнопок "Удалить"
    setupDeleteButtons();
}
...
function createEditableTable(data) {
    ...
    const headers = [
        'Имя', 'Класс', 'Информатика', 'Физика', 'Математика', 'Литература', 'Музыка', ' '
    ]; // Заголовки таблицы
    ...
    data.forEach((row, rowIndex) => {
        ...
        html += `<td>
                    <button class="edit-btn" data-row-index="${rowIndex}">Edit</button>
                    <button class="save-btn" data-row-index="${rowIndex}" style="display: none;">Save</button>
                    <button class="delete-btn" data-row-index="${rowIndex}">Delete</button>
                </td>`;
        html += '</tr>';
    });
